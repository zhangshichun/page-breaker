...
export default function PageBreaker({ imgUrl }: { imgUrl: string }) {
  ...
  return (
    <div className='container' ref={containerRef}>
      {
        finished ? <Button className='center-btn' onClick={onBtnClick}>重新轰炸！</Button> : null
      }
      {
        !finished ? <>
          <Firecrackers className="center" onBoom={onBoom}></Firecrackers>
          {width && height && <ImageBreak ref={breaker} canvasWidth={width} canvasHeight={height} imgUrl={imgUrl} onFinished={onFinished}/>}
        </> : null
      }
